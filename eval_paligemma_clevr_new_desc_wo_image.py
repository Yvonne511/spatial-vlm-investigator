...
for i in tqdm(range(0, len(dataset), batch_size)):
    ...
    for question, prompt, answer in zip(questions, prompts, gt_answers):
        tokens, mask_ar, mask_loss, mask_input = preprocess_tokens(
            # prefix=f"{prompt}\nQuestion: {question}", 
            prefix = f"{prompt}\nContext: The scene is described by spatial order of objects. Analyze the provided left-to-right and front-to-back descriptions. Then, respond to the question.\nQuestion: {question}",
            suffix=None, 
            seqlen=SEQLEN
        )
    ...
    batch_dict = {
        ...
        "image":np.zeros((batch_size, 448, 448, 3), dtype=np.float16),
