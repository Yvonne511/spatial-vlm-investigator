...
dataset = vsr_dataset_random["dev"].select(range(1000))
batch_size = 20
...
def format_question(q):
    q = q.strip()
    return q if q.endswith("?") else q + " Is this true?"

# SEQLEN = 128
SEQLEN = 128
responses = []
ground_truths = []
image_folder = "/vast/yw4142/datasets/vsr/images"
for i in tqdm(range(0, len(dataset), batch_size)):
    ...
    questions = [format_question(cap) for cap in captions]
    gt_answers_raw = batch["label"] # 0 or 1
    gt_answers = ["yes" if label == 1 else "no" for label in gt_answers_raw]

    # Preprocess images
    images = [
        preprocess_image(Image.open(os.path.join(image_folder, fname)).convert("RGB"))
...
# Accuracy: 72.50% for 1000 samples
