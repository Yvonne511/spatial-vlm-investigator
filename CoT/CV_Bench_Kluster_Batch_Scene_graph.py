...
def extract_json_from_response(full_response):
    """
    Attempts to extract valid JSON from a response string using multiple strategies, prioritizing the last match.
    ...
    """
    try:
        # Strategy 4: Extract content from the LAST code block
        code_block_matches = list(re.finditer(r'```(?:json)?\s*([\s\S]*?)\s*```', full_response, re.IGNORECASE))
        if code_block_matches:
            last_match = code_block_matches[-1]
            potential_json = last_match.group(1).strip()
            try:
        ...
        # Strategy 2: Look for the LAST JSON object pattern with regex
        json_matches = list(re.finditer(r'({[\s\S]*?})(?:\s*$|\n)', full_response))
        if json_matches:
            last_json_match = json_matches[-1]
            json_str = last_json_match.group(1)
            try:
                json.loads(json_str)
                return json_str
            except:
        ...
        # Strategy 3: Look for content after the LAST "Scene Graph:" label
        sg_matches = list(re.finditer(r'Scene Graph:?\s*([\s\S]*)', full_response, re.IGNORECASE))
        if sg_matches:
            last_sg_match = sg_matches[-1]
            potential_json = last_sg_match.group(1).strip()
            try:
                json.loads(potential_json)
                return potential_json
            except:
        ...
        answer_request = {
            ...
            "body": {
                ...
                "messages": [
                    {
                        "role": "system",
                        "content": "You are an AI assistant proficient in Visual and Spatial reasoning with tasks involving counting, relations, depth, distances, etc. Think based on the image and the JSON scene graph and then answer. Final answer should be provided between <answer> </answer> tags. All questions are multiple choice, provide the letter (A, B, C, or D) of the correct answer after thinking."
                    },
                    {
                        "role": "user",
                        "content": [
